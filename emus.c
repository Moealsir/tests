...
int _exitshell(info_t *info)
{
	...
	{
		exitcheck = _errstrtoin(info->argv[1]);
		if (exitcheck == -1)
		{
			info->status = 2;
			_error(info, "Illegal number: ");
			_input_string(info->argv[1]);
			writs_to_stderr('\n');
			return (1);
		}
		info->err_num = _errstrtoin(info->argv[1]);
		return (-2);
	}
	info->err_num = -1;
	return (-2);
...
int custom_cd(info_t *info)
{
	...
	if (chdir_ret == -1)
	{
		...
		_input_string(info->argv[1]), writs_to_stderr('\n');
	}
	else
	{
