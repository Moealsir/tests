...
int _my_setenv(info_t *info)
{
	if (info->argc != 3)
	{
		_input_string("Incorrect number of arguements\n");
		return (1);
	}
...
int _my_unsetenv(info_t *info)
{
	...
	if (info->argc == 1)
	{
		_input_string("Too few arguements.\n");
		return (1);
	}
...
int populate_envlist(info_t *info)
{
	...
	for (i = 0; environ[i]; i++)
		add_a_node_end(&node, environ[i], 0);
	info->env = node;
	return (0);
}
